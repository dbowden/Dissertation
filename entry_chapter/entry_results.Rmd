...
m2 <- glm(new.joiner ~ latentmean_diff + ethfrac  + maxint + cont_civil + Incompatibility + multireb + lgdp + lpop + mtnest + polity2 + postcw, data=data.frame(conflict.year), family=binomial("logit"))
m2c <- coeftest(m2, vcov. = vcovHC(m2, type="HC1", cluster="GWNoA"))

m3 <- glm(new.joiner ~ latentmean_diff*ethfrac  + maxint + cont_civil + multireb + lgdp + lpop + mtnest + polity2 + Incompatibility + postcw, data=data.frame(conflict.year), family=binomial("logit"))
...
m4 <- glm(new.joiner ~ latentmean_diff + maxint + cont_civil + multireb + GWNoA + yearf, data=data.frame(conflict.year), family=binomial("logit"))
...
texreg(list(m1,m2,m3, m4), custom.coef.map=map, caption="Logit Models of Rebel Group Formation")
```
...
```{r effectplot, fig.cap="Predicted Probability of New Rebel Group Formation"}
library(sjPlot)
library(ggplot2)
...
c1 <- glm(monoeth ~ joiner + splinter + alliance + Incompatibility + prevactive + country_ethnic + (rebpresosts!="no"), data=group, family=binomial("logit"))

c2 <- glm(multieth ~ joiner + splinter + alliance + Incompatibility + prevactive + country_ethnic + (rebpresosts!="no"), data=group, family=binomial("logit"))

c3 <- glm(noneth ~ joiner + splinter + alliance + Incompatibility + prevactive + country_ethnic + (rebpresosts!="no"), data=group, family=binomial("logit"))

texreg(list(c1,c2,c3), caption="Logit Models of Rebel Group Ethnic Composition", custom.model.names=c("M1 Monoethnic", "M2 Multiethnic", "M3 Nonethnic"))
```